...
#		docker build --tag clients-manager "$clients_manager_path"
	...
	"install" )
		...
		chmod +x "$script_path"
		ln -sf "$script_path" /bin/watchwolf # run WatchWolf from any place
		...
		if [ $no_startup -eq 0 ]; then
			...
			if [ $wsl -eq 0 ]; then
				...
				bash -c "echo '$service_contents' > /etc/systemd/system/watchwolf.service" # create service
				systemctl enable watchwolf # init service
			else
				# WSL
				...
				if [ `echo "$option" | grep -i -E -c '^[DK]'` -ne 0 ]; then # Exit?
					if [ `echo "$option" | grep -i -E -c '^D'` -ne 0 ]; then
						# don't keep; disable password
						bash -c "echo '`whoami` ALL=(ALL) NOPASSWD:ALL' > /etc/sudoers.d/`whoami`" && chmod 0440 /etc/sudoers.d/`whoami` # @ref https://www.folkstalk.com/tech/ubuntu-wsl-disable-sudo-password-prompt-with-code-examples/
	...
	"uninstall" )
		...
		if [ $wsl -eq 0 ]; then
			...
			rm /etc/systemd/system/watchwolf.service
		else
			...
			rm "$windows_start_folder/WatchWolf.bat"
		fi
		;;
